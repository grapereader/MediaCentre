...
namespace vmc
{
    namespace auth
    {
        ...
        bool comparePassword(std::string const &hash, std::string const &password)
        {
            ...
            std::string saltString = parts[0];
            std::string keyString = parts[1];

            CryptoPP::StringSource saltSource(saltString, true, new CryptoPP::Base64Decoder(new CryptoPP::StringSink(saltString)));

            CryptoPP::SecByteBlock key(CryptoPP::SHA256::DIGESTSIZE);
            CryptoPP::PKCS5_PBKDF2_HMAC<CryptoPP::SHA256> pbkdf;

            pbkdf.DeriveKey(key, key.size(), 0, (byte *) password.data(), password.size(), (byte *) saltString.data(), saltString.size(), ITERATIONS);
